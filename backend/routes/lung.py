"""BioVision AI - Lung Cancer Route"""
from flask import Blueprint, request, jsonify
from utils.image_utils import validate_image_file, load_image_from_bytes
from services.lung_service import predict_lung
from services.chat_service import generate_medical_recommendation
import logging

logger  = logging.getLogger(__name__)
lung_bp = Blueprint('lung', __name__)

@lung_bp.route('/lung', methods=['POST'])
def lung_predict():
    try:
        if 'file' not in request.files:
            return jsonify({"error": "No file provided"}), 400

        file = request.files['file']
        valid, msg = validate_image_file(file)
        if not valid:
            return jsonify({"error": msg}), 400

        img    = load_image_from_bytes(file.read())
        result = predict_lung(img)
        ctx    = f"Lung cancer CT scan: {result['prediction']} with {result['confidence']}% confidence."
        result['ai_recommendation'] = generate_medical_recommendation(ctx)
        return jsonify(result)

    except Exception as e:
        logger.error(f"Lung prediction error: {e}")
        return jsonify({"error": str(e)}), 500